...
if (str (findDisplay 5601) == "No display") then {createDialog "DMNS_UnscDataBase";};
...
if (typeName (_this select 0) == "object") then {
	...
	[true,(lbData [1501,(lbCurSel 1501)])] call DMNS_fnc_UNSCdatabase;
	...
	if (typeName (_this select 0) == "bool") then {
		...
		switch _topic do {
			...
			case "DMNS Developers":
			{
				{
					_i = _topicEntrys lbAdd _x;
					_topicEntrys lbSetData [_i,_x];
				} forEach ((configfile >> "DMNS_Wiki_DevProfiles") call BIS_fnc_getCfgSubClasses);
			};
			...
			{
				{
					...
					//"DMNS_Longsword_AA_F","DMNS_Longsword_BOMB_F","DMNS_Longsword_CANNON_F","DMNS_Longsword_CAS_F","DMNS_Longsword_SUPPRESS_F",
		...
		switch _topic do {
			...
			case "DMNS Developers":
			{
				_topicPicture ctrlSetText (getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "profilePicture"));
				_topicPictureLong ctrlSetText "";
				_topicTittle ctrlSetText (format ["Subjects Pseudonym: %1",(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "pseudonym"))]);
				_topicLong ctrlSetText (
					format [
						"\n%1\n\n%2\n\n%3",
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "bioParagraph1")),
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "bioParagraph2")),
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "bioParagraph3"))
					]
				);
				_topicShort ctrlSetText (
					format [
						"TITTLE: %1\nFIRSTNAME: %2\nSURNAME: %3\nSEX: %4\nDOB: %5\nHOMEWORLD: %6\nNATIONALITY: %7\nALLEGIANCE: %8\nROLE: %9",
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "tittle")),
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "firstName")),
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "secondName")),
						//(getText (configfile >> "OPTRE_Wiki_DevProfiles" >> _topicEntry >> "pseudonym")),
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "sex")),
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "DOB")),
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "homeWorld")),
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "nationality")),
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "Allegiance")),
						//(getText (configfile >> "OPTRE_Wiki_DevProfiles" >> _topicEntry >> "Affiliations")),
						(getText (configfile >> "DMNS_Wiki_DevProfiles" >> _topicEntry >> "role"))
					]
				);
			};
			case "UNSC Ground Vehicles":
