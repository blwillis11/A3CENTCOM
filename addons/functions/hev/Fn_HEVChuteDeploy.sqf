...
private _chute = "DMNS_SOEIV_Chute" createVehicle [0,0,0];
...
[
    {(getPosATLVisual (_this select 0) select 2) <= (_this select 3)},
    {
        ...
        _chute attachTo [_hev, [0.0,0.0,1.70]];;

        if (_hev in _hevArrayPlayer) then {
            [_chute,_hev] call DMNS_fnc_PlayerHEVEffectsUpdate_Chute;

            [_hev] call DMNS_fnc_PlayerHEVEffectsUpdate_GroundAlarm;
        };
        ...
        _chute animate ["chute_rotation",50];
...
[
...

    {
        ...
        if (_hev isEqualTo _lastPod) then {
            private _deleteReadyString = ["DMNS_HEV_deleteReady",_HEVLaunchNumbertring] joinString "_";
