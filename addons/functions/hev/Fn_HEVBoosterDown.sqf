...
[0,_hev,_light] remoteExecCall ["DMNS_fnc_PlayerHEVEffectsUpdate_Light", _listOfPlayers, false];

private _smoke = "#particlesource" createVehicle [0,0,0];
_smoke setParticleClass "Missile2";
_smoke attachto [_hev,[0,0.0,0.6]];
_hev animate ["ThrusterFX_hide",1];
_hev animate ["ThrusterFX_rotation",15];
_hev animate ["ThrusterFX_rotation2",15];
_hev animate ["ThrusterFX_translation_a",1];
_hev animate ["ThrusterFX_translation_b",1];
_hev animate ["ThrusterFX_translation_c",1];
_hev animate ["ThrusterFX_translation_d",1];
_hev animate ["ThrusterFX_translation_e",1];
_hev animate ["ThrusterFX_translation_f",1];
_hev animate ["ThrusterFX_translation_g",1];

private _boosterLights = [_smoke,_light];


if ((gunner _hev) in (call CBA_fnc_players)) then {
    [(random _randomXYVelocity),(random _randomXYVelocity),_launchSpeed,_hev,_hevDropArmtmosphereStartHeight] call DMNS_fnc_PlayerHEVEffectsUpdate_BoasterDown;
} else {
    ...
    [
        {
            ...
            private _deleteShipString = ["DMNS_HEV_deleteShipEvent",str _HEVLaunchNumber] joinString "_";
...
_hev animate ["ThrusterFX_hide",0];
_hev animate ["ThrusterFX_rotation",0];
_hev animate ["ThrusterFX_rotation2",0];
_hev animate ["ThrusterFX_translation_a",0];
_hev animate ["ThrusterFX_translation_b",0];
_hev animate ["ThrusterFX_translation_c",0];
_hev animate ["ThrusterFX_translation_d",0];
_hev animate ["ThrusterFX_translation_e",0];
_hev animate ["ThrusterFX_translation_f",0];
_hev animate ["ThrusterFX_translation_g",0];
